...
describe('ContextWindowSideBar Component', () => {
    ...
    const mockMessages = [
        ...
        },
        {
            ...
            components: [
                {
                    type: 'text',
                    content: 'The context window allows you to control how much of the conversation history is included when sending a new message to the AI.'
                }
            ]
        }
    ];
    ...
    it('shows different icons for different AI models', () => {
        ...
        const gptMessage = Array.from(messageItems).find(
            el => el.querySelector('.message-header .role-label')?.textContent?.includes('GPT')
        );
        
        expect(gptMessage).not.toBeNull();
        
        // Find the Claude message
        const claudeMessage = Array.from(messageItems).find(
            el => el.querySelector('.message-header .role-label')?.textContent?.includes('Claude')
        );
