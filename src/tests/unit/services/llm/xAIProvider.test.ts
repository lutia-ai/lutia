			...
			chat: {
				completions: {
					create: vi.fn().mockImplementation((params) => {
						const hasReasoning = params.reasoning_effort === 'high';
...
vi.mock('$env/dynamic/private', () => ({
	env: {
		SECRET_XAI_API_KEY: 'test-api-key'
	}
}));
...
describe('XAIProvider', () => {
	...
	it('should create completion stream with reasoning enabled using reasoning_effort', async () => {
		...
		// Check that the OpenAI mock was called with reasoning_effort: 'high'
		const OpenAIModule = await import('openai');
		const mockOpenAI = vi.mocked(OpenAIModule.default);

		// Access the mock instance created during the test
	...
	it('should handle error in stream creation', async () => {
		...
		console.log('[xAI Provider] Testing error handling for:', errorMessage);
	...
	it('should handle empty content chunks and call onFirstChunk', async () => {
		const emptyChunk = {
			choices: [
				{
					index: 0,
	...
	it('should use correct base URL for xAI API', async () => {
	...
	it('should not add reasoning_effort when reasoning is disabled', async () => {
		const messages = [{ role: 'user', content: 'Hello' }];
		...
		// Check that the OpenAI mock was called without reasoning_effort
		const OpenAIModule = await import('openai');
		const mockOpenAI = vi.mocked(OpenAIModule.default);
		...
		// The mock should not have been called with reasoning_effort
		expect(mockInstance.chat.completions.create).toHaveBeenCalledWith(
