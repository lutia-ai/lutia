...
// Mock OpenAI (Llama uses OpenAI SDK)
vi.mock('openai', () => {
	...
	return {
		default: vi.fn().mockImplementation(() => ({
			chat: {
				completions: {
					create: vi.fn().mockResolvedValue(createMockStream())
				}
			}
		}))
	};
...
vi.mock('$env/dynamic/private', () => ({
	env: {
		VITE_LLAMA_API_KEY: 'test-api-key'
	}
}));
...
describe('LlamaProvider', () => {
	...
	beforeEach(() => {
		...
		mockModel = {
			name: 'Llama_3_2_90b_Vision' as any,
			param: 'llama-3.2-90b-vision',
			legacy: false,
	...
	it('should handle error in stream creation', async () => {
		// Test error handling by creating a provider that will fail
		const errorMessage = 'API Error';
		console.log('[Llama Provider] Testing error handling for:', errorMessage);

		// This test verifies that the provider can handle errors gracefully
	...
	it('should handle empty content chunks', async () => {
		...
		// Should not call onContent for empty chunks
		expect(mockCallbacks.onContent).not.toHaveBeenCalled();
	});

	it('should handle chunk marked as first chunk', async () => {
		const firstChunk = {
			isFirstChunk: true,
			choices: [
				{
		...
		provider.handleStreamChunk(firstChunk, mockCallbacks);

		// Should call onFirstChunk for first chunk
	...
	it('should handle chunk with index 0', async () => {
		const indexZeroChunk = {
			choices: [
				{
					index: 0,
					delta: {
		...
		provider.handleStreamChunk(indexZeroChunk, mockCallbacks);

		// Should call onFirstChunk for index 0 chunk
		expect(mockCallbacks.onFirstChunk).toHaveBeenCalledWith(
			'123e4567-e89b-12d3-a456-426614174000',
			''
		);
	...
	it('should handle a complete stream lifecycle', async () => {
		...
		// Should have called onFirstChunk at least once (Llama calls it for each content chunk)
	...
	it('should use correct base URL for Llama API', async () => {
		// Access the OpenAI mock to verify it was called with correct params
	...
	it('should not handle reasoning content since Llama does not support reasoning', async () => {
		const chunk = {
			choices: [
				{
					index: 0,
		...
		// Should call onContent but not onReasoning
		expect(mockCallbacks.onContent).toHaveBeenCalledWith('Hello');
