...
describe('GeminiProvider', () => {
	...
	it('should report usage on final chunk', async () => {
		// Create a mock final chunk with usage data
		const finalChunk = {
			isLast: true,
			usage: {
				input_tokens: 10,
		...
		// Call handleStreamChunk with the final chunk
	...
	it('should handle a complete stream lifecycle', async () => {
		...
		// The final chunk with usage should have been generated by normalizeGeminiStream
		expect(mockCallbacks.onUsage).toHaveBeenCalled();
	});

	it('should handle errors in token counting', async () => {
		// Mock an error in countTokens
