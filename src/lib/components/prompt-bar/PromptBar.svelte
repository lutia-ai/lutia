<script lang="ts">
    ...
    import type { FileAttachment, Image, Message, Model, UserWithSettings } from '$lib/types/types';
    import {
        chatHistory,
        numberPrevMessages,
        chosenCompany,
    ...
    $: if (
        ...
        if (mounted) {
            ...
            if (fileAttachments.length > 0) {
                ...
                calculateTokensAndPrice($fullPrompt, imagePreview, $chosenModel, $chosenCompany)
                    .then(result => {
    ...
    $: promptBarWrapperWidth = $isSidebarOpen && !$mobileSidebarOpen && $isLargeScreen
        ? 'calc(100% - 310px)'
        : '';
    ...
    function handleKeyDown(event: CustomEvent<KeyboardEvent>) {
        ...
        if (showModelSearch) {
            if (e.key === 'Enter') {
                ...
                e.preventDefault();
                selectedModelIndex = selectedModelIndex === null || 
                                     selectedModelIndex === filteredModels.length - 1
                    ? 0
                    : selectedModelIndex + 1;
            } else if (e.key === 'ArrowUp') {
                e.preventDefault();
                selectedModelIndex = selectedModelIndex === null || selectedModelIndex === 0
    ...
    async function handleSubmit() {
        ...
        // Reset token counts
        ({ tokens: input_tokens, price: input_price } = await calculateTokensAndPrice(prompt, imagePreview, $chosenModel, $chosenCompany));
    }
