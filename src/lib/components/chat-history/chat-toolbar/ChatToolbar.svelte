...
<div class="chat-toolbar-container" style="opacity: {chat.price_open ? '1' : ''};">
	<div
		class="toolbar-item"
		role="button"
		tabindex="0"
	...
	>
		...
		{/if}
		<HoverTag text={chat.copied ? 'copied' : 'copy'} position="bottom" distance={12} />
	</div>
	<div
		class="toolbar-item"
		role="button"
	...
	<div
		...
		on:click|stopPropagation={async () => {
			await regenerateMessage(chat.message_id ?? 0);
		}}
	...
	>
		...
		<span class="model-name">{formatModelEnumToReadable(chat.by)}</span>
		<HoverTag text={'Regenerate response'} position="bottom" distance={12} />
	</div>
</div>
