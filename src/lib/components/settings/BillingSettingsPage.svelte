...
<div class="billing-body">
    ...
    {#if addCreditOpen}
        <form
            class="top-up-open"
            on:submit|preventDefault={handleTopupSubmit}
        >
            <div class="left">
                <h1>Add funds</h1>
                <p
                    style="color: {(addCreditAmount >= 5 && addCreditAmount <= 100) || addCreditAmount === undefined || addCreditAmount === 0
                        ? '' 
                        : 'red'}"
                >
    ...
    <div class="card-body">
        ...
        {#if !showCardInput}
            <div class="card-info {loading ? 'shimmerBG' : 'container-bg'}">
                {#if cardDetails && !showCardInput}
                    ...
                    <div class="sub">
                        <p class="card-expiry">Expires: {formatExpiryDate(cardDetails.expMonth, cardDetails.expYear)}</p>
                        <div 
                            class="update"
                            role="button"
                            tabindex="0"
        ...
        >
            ...
            {#if cardDetails}
                <button
                    class="cancel"
                    on:click|preventDefault={() => showCardInput = false }
                >
                    Cancel
                </button>
            {/if}
...
<style lang="scss">
    ...
    .shimmerBG {
        ...
        background: var(--bg-color-light-opacity-alt);
        background: linear-gradient(to right, var(--bg-color-light-opacity-alt) 8%, #F0F0F0 18%, var(--bg-color-light-opacity-alt) 33%);
