 ...
 */
export async function retrieveConversationById(conversationId: string): Promise<Conversation | null> {
    try {
 ...
 */
export async function verifyConversationOwnership(conversationId: string, userId: number): Promise<boolean> {
    try {
        const conversation = await prisma.conversation.findUnique({
            where: {
                id: conversationId
