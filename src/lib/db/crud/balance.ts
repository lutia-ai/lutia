...
export async function updateUserBalanceWithDeduction(userId: number, deductionAmount: number): Promise<number> {
    try {
...
export async function updateUserBalanceWithIncrement(userId: number, incrementAmount: number): Promise<number> {
    try {
        const updatedBalanceRecord = await prisma.balance.update({
            where: { user_id: userId },
            data: {
