...
export async function POST({ request, locals, url }) {
    ...
    try {
        ...
        console.log(`[LLM API] Request body:`, JSON.stringify({
            provider: requestBody.provider,
            regenerateMessageId: requestBody.regenerateMessageId,
            hasImages: requestBody.imagesStr ? JSON.parse(requestBody.imagesStr).length > 0 : false,
            hasFiles: requestBody.filesStr ? JSON.parse(requestBody.filesStr).length > 0 : false,
        ...
        console.log(`[LLM API] Model: ${model.name}, Conversation: ${messageConversationId || 'new'}`);
