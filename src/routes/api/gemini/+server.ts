	...
	try {
		...
		const readableStream = new ReadableStream({
			async start(controller) {
                ...
                const outputCountResult = await genAIModel.countTokens(
                    completeResult
                );
                // @ts-ignore
                outputCountResult.price = (outputCountResult.totalTokens / 1000000) * model.output_price;
                ...
                try {
                    ...
                    await fs.writeFile('token_comparison.json', JSON.stringify(existingData, null, 2)); // Pretty-print for readability
