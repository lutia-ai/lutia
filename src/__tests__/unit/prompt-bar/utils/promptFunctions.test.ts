...
const mockAddFilesToMessage = vi.fn((messages, files) => {
    ...
    return messages.map((msg: Message) => ({
        ...msg,
        content: msg.content + files.map((f: { filename: string }) => ` [FILE:${f.filename}]`).join('')
    }));
});

const mockIsUserChatComponent = vi.fn().mockImplementation((component: ChatComponent): boolean => component.by === 'user');
    ...
    describe('generateFullPrompt', () => {
        ...
        it('should handle file attachments in user messages', () => {
            ...
            mockIsUserChatComponent.mockImplementation((component: ChatComponent): boolean => {
                return component.by === 'user' && 'attachments' in component && 
                    component.attachments !== undefined && component.attachments.length > 0;
            });
            
            mockAddFilesToMessage.mockImplementationOnce((messages: Message[], files: FileAttachment[]) => {
                return [{ 
                    ...messages[0], 
            ...
            const userMsgWithFile = result.find(msg => 
                msg.content && msg.content.includes('[FILE:example.js]')
            );
